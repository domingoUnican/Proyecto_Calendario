    ...
    def toXHSTT(self, out=OUT, iden=ASIG):
        ...
        for asign in self.asig:
            ...
            for temp_course in range(1, int(N_GROUPS[self.asig[asign][0]]) + 1):
                for i, degree, special in self.asig[asign][-1]:
                    key = 'Class%s_%s_%s%s' % (
                        str(i), degree, special, str(temp_course))
                    key = key.replace(' ', '_')
                    courses.append(key)
            ...
            list_prof = [
                p for p in self.prof if asign in self.prof[p][0].split(',')]
            for p in list_prof:
                resource = etree.SubElement(resources, 'Resource',
        ...
        for asign in self.asig:
            for temp_i in range(1, N_GROUPS[self.asig[asign][0]] + 1):
                ...
                for i, degree, special in self.asig[asign][-1]:
                    key = 'Class%s_%s_%s%s' % (
                        str(i), degree, special, str(temp_i))
                    key = key.replace(' ', '_')
                    courses.append(key)
        ...
        event_groups = etree.SubElement(AppliesTo, 'EventGroups')
        etree.SubElement(event_groups, 'EventGroup', Reference='gr_AllEvents')
        prefer = etree.SubElement(
            constraints, 'PreferTimesConstraint', Id='manana')
        ...
        avoid = etree.SubElement(
            constraints, 'AvoidClashesConstraint', Id='Choques')
        ...
        for recurso in RECURSOS:
            resource = etree.SubElement(
                resource_groups, 'ResourceGroup', Reference='gr_' + recurso)
